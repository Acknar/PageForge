...
def default_output_name():
    """Name of the auto-created output folder beside the user's files —
    localized (French → « Résultat »)."""
    return "Résultat" if LANG == "fr" else "output"
...
class Section(QWidget):
    ...
    def __init__(self, title, on_activate):
        ...
        # The gap sits OUTSIDE the styled card (on the Section wrapper), so cards
        # never touch and a COLLAPSED card is exactly its header — the title
        # stays vertically centred with no empty sliver below it.
        outer = QVBoxLayout(self)
        outer.setContentsMargins(0, 0, 0, 14)
        outer.setSpacing(0)

        self.card = QFrame()
        self.card.setObjectName("pfSection")
...
class MainWindow(QMainWindow):
    ...
    def _build_window_content(self):
        ...
        splitter.setHandleWidth(2)               # a thin divider line, no frame
    ...
    def _build_preview(self):
        ...
        self.zone_hint.setStyleSheet("padding:4px 10px;")  # plain text, no background
